...
@section('content')
    <div class="space-y-6">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <h1 class="text-2xl sm:text-3xl lg:text-4xl font-bold break-words">Edit Vehicle Type</h1>
                <p class="text-gray-600 mt-1">Update details for {{ $vehicleType->name }}.</p>
            </div>
        ...
        <x-shadcn.card class="shadow-lg">
            <x-shadcn.card-content class="p-6">
                <form method="POST" action="{{ route('admin.vehicle-types.update', $vehicleType) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <x-text-input id="name" class="block mt-1 w-full" type="text" name="name"
                                    :value="old('name', $vehicleType->name)" required autofocus />
                                ...
                                <div class="flex justify-between items-center">
                                    ...
                                    @if(auth()->user()->isSuperAdmin() || auth()->user()->hasPermission('transfers', 'create'))
                                        <a href="{{ route('admin.vehicle-classes.create') }}"
                                            class="text-xs text-logo-link hover:underline flex items-center">
                                            <i data-lucide="plus" class="w-3 h-3 mr-1"></i>
                                            Add New Class
                                        </a>
                                    @endif
                                </div>
                                <select id="vehicle_class_id" name="vehicle_class_id"
                                    ...
                                    @foreach($vehicleClasses as $class)
                                        <option value="{{ $class->id }}" {{ old('vehicle_class_id', $vehicleType->vehicle_class_id) == $class->id ? 'selected' : '' }}>
                                            {{ $class->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <x-text-input id="max_passengers" class="block mt-1 w-full" type="number"
                                    name="max_passengers" :value="old('max_passengers', $vehicleType->max_passengers)"
                                    required min="1" />
                            ...
                            <div>
                                ...
                                <x-text-input id="max_luggages" class="block mt-1 w-full" type="number" name="max_luggages"
                                    :value="old('max_luggages', $vehicleType->max_luggages)" required min="0" />
                    ...
                    <div class="flex items-center justify-end mt-6">
                        <a href="{{ route('admin.vehicle-types.index') }}" class="text-gray-600 hover:text-gray-900 mr-4">
                            Cancel
                        </a>
