                ...
                <form method="GET" action="{{ route('admin.users.index') }}" class="flex flex-col sm:flex-row gap-4">
                    ...
                    <div>
                        <select name="role"
                            ...
                            <option value="organizer" {{ request('role') === 'organizer' ? 'selected' : '' }}>Organizers
                            </option>
                        </select>
                    </div>
                    <button type="submit"
                    ...
                    <x-shadcn.table-body>
                        @forelse($users as $user)
                            <x-shadcn.table-row hover>
                                ...
                                <x-shadcn.table-cell>
                                    <x-shadcn.badge variant="{{ $user->is_active ? 'default' : 'destructive' }}"
                                        class="{{ $user->is_active ? 'bg-green-100 text-green-800 hover:bg-green-100' : '' }}">
                                    ...
                                    <div class="flex flex-wrap items-center gap-2 text-xs sm:text-sm">
                                        ...
                                        @can('update', $user)
                                            <form method="POST" action="{{ route('admin.users.toggle-active', $user) }}"
                                                ...
                                                @method('PATCH')
                                                <button type="submit"
                                                    class="{{ $user->is_active ? 'text-orange-600 hover:text-orange-900' : 'text-green-600 hover:text-green-900' }} whitespace-nowrap">
                        ...
                        @empty
                            <x-shadcn.table-row>
                                <x-shadcn.table-cell colspan="7" class="text-center text-muted-foreground">No users
