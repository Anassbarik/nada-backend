...
@section('content')
    <div class="space-y-6">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <h1 class="text-2xl sm:text-3xl lg:text-4xl font-bold break-words">Edit Transfer</h1>
        ...
        <x-shadcn.card class="shadow-lg">
            <x-shadcn.card-content class="p-6">
                <form method="POST" action="{{ route('admin.transfers.update', [$accommodation, $transfer]) }}"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            ...
                            <div>
                                ...
                                <x-text-input id="client_email" class="block mt-1 w-full" type="email" name="client_email"
                                    :value="old('client_email', $transfer->client_email)" />
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            <div>
                                ...
                                <select id="transfer_type" name="transfer_type"
                                    ...
                                    required>
                                    <option value="airport_hotel" {{ old('transfer_type', $transfer->transfer_type) === 'airport_hotel' ? 'selected' : '' }}>Airport to Hotel
                                    </option>
                                    <option value="hotel_airport" {{ old('transfer_type', $transfer->transfer_type) === 'hotel_airport' ? 'selected' : '' }}>Hotel to Airport
                                    </option>
                                    <option value="hotel_event" {{ old('transfer_type', $transfer->transfer_type) === 'hotel_event' ? 'selected' : '' }}>Hotel to Event
                                    </option>
                                    <option value="event_hotel" {{ old('transfer_type', $transfer->transfer_type) === 'event_hotel' ? 'selected' : '' }}>Event to Hotel
                                    </option>
                                    <option value="city_transfer" {{ old('transfer_type', $transfer->transfer_type) === 'city_transfer' ? 'selected' : '' }}>City Transfer
                                    </option>
                                </select>
                            ...
                            <div>
                                ...
                                <select id="trip_type" name="trip_type"
                                    class="block mt-1 w-full bg-white text-gray-900 border-gray-300 rounded-md shadow-sm"
                                    required>
                                    <option value="one_way" {{ old('trip_type', $transfer->trip_type) === 'one_way' ? 'selected' : '' }}>Aller Simple (One Way)</option>
                                    <option value="round_trip" {{ old('trip_type', $transfer->trip_type) === 'round_trip' ? 'selected' : '' }}>Aller-Retour (Round Trip)</option>
                                </select>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            <div>
                                ...
                                <x-text-input id="transfer_date" class="block mt-1 w-full" type="date" name="transfer_date"
                                    :value="old('transfer_date', $transfer->transfer_date ? \Carbon\Carbon::parse($transfer->transfer_date)->format('Y-m-d') : '')" required />
                            ...
                            <div>
                                ...
                                <x-text-input id="pickup_time" class="block mt-1 w-full" type="time" name="pickup_time"
                                    :value="old('pickup_time', $transfer->pickup_time ? \Carbon\Carbon::parse($transfer->pickup_time)->format('H:i') : '')" required />
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            <div>
                                ...
                                <x-text-input id="pickup_location" class="block mt-1 w-full" type="text"
                                    name="pickup_location" :value="old('pickup_location', $transfer->pickup_location)"
                                    required placeholder="e.g., CMN Airport" />
                            ...
                            <div>
                                ...
                                <x-text-input id="dropoff_location" class="block mt-1 w-full" type="text"
                                    name="dropoff_location" :value="old('dropoff_location', $transfer->dropoff_location)"
                                    required placeholder="e.g., Hyatt Regency" />
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            <div>
                                ...
                                <x-text-input id="flight_number" class="block mt-1 w-full" type="text" name="flight_number"
                                    :value="old('flight_number', $transfer->flight_number)" placeholder="e.g., AT222" />
                            ...
                            <div>
                                ...
                                <x-text-input id="flight_time" class="block mt-1 w-full" type="time" name="flight_time"
                                    :value="old('flight_time', $transfer->flight_time ? \Carbon\Carbon::parse($transfer->flight_time)->format('H:i') : '')" />
                                ...
                                <select id="vehicle_type_id" name="vehicle_type_id"
                                    ...
                                    @foreach($vehicleTypes as $type)
                                        <option value="{{ $type->id }}"
                                            ...
                                            {{ old('vehicle_type_id', $transfer->vehicle_type_id) == $type->id ? 'selected' : '' }}>
                    ...
                    <div class="mb-6 return-transfer-section" id="return-transfer-section"
                        style="display: {{ old('trip_type', $transfer->trip_type) === 'round_trip' ? 'block' : 'none' }};">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            <div>
                                ...
                                <x-text-input id="return_date" class="block mt-1 w-full" type="date" name="return_date"
                                    :value="old('return_date', $transfer->return_date ? \Carbon\Carbon::parse($transfer->return_date)->format('Y-m-d') : '')" />
                            ...
                            <div>
                                ...
                                <x-text-input id="return_time" class="block mt-1 w-full" type="time" name="return_time"
                                    :value="old('return_time', $transfer->return_time ? \Carbon\Carbon::parse($transfer->return_time)->format('H:i') : '')" />
                    ...
                    <div class="mb-6">
                        ...
                        <div class="mb-4">
                            ...
                            <p class="mt-1 text-sm text-gray-500">PDF or images (JPG, PNG, WebP) - max 10MB. Leave empty to
                                keep current file.</p>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            <div>
                                ...
                                <select id="status" name="status"
                                    class="block mt-1 w-full bg-white text-gray-900 border-gray-300 rounded-md shadow-sm"
                                    required>
                                    <option value="pending" {{ old('status', $transfer->status) === 'pending' ? 'selected' : '' }}>Pending (Not Paid)</option>
                                    <option value="paid" {{ old('status', $transfer->status) === 'paid' ? 'selected' : '' }}>
                                        Paid</option>
                                    <option value="confirmed" {{ old('status', $transfer->status) === 'confirmed' ? 'selected' : '' }}>Confirmed</option>
                            ...
                            <div>
                                ...
                                <select id="payment_method" name="payment_method"
                                    ...
                                    <option value="wallet" {{ old('payment_method', $transfer->payment_method) === 'wallet' ? 'selected' : '' }}>Wallet</option>
                                    <option value="bank" {{ old('payment_method', $transfer->payment_method) === 'bank' ? 'selected' : '' }}>Bank Transfer</option>
                                    <option value="both" {{ old('payment_method', $transfer->payment_method) === 'both' ? 'selected' : '' }}>Both (Wallet + Bank)</option>
                                </select>
