    ...
    <x-shadcn.card class="shadow-lg">
        <x-shadcn.card-content class="p-6">
            <form method="POST" action="{{ route('admin.admins.update', $admin) }}">
                ...
                <div id="permissions-section" class="mb-6" style="display: {{ old('role', $admin->role) === 'admin' ? 'block' : 'none' }};">
                    <x-input-label value="Permissions" />
                    <p class="text-sm text-gray-500 mb-4">Select the permissions for this admin:</p>
                    
                    @php
                        $adminPermissionIds = $admin->permissions->pluck('id')->toArray();
                    @endphp
                                ...
                                @foreach($permissions as $permission)
                                    <label class="flex items-center">
                                        <input type="checkbox" 
                                               ...
                                               {{ in_array($permission->id, old('permissions', $adminPermissionIds)) ? 'checked' : '' }}>
