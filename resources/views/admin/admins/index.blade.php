                    ...
                    <x-shadcn.table-body>
                        @forelse($admins as $admin)
                            <x-shadcn.table-row hover>
                                ...
                                <x-shadcn.table-cell>
                                    <x-shadcn.badge variant="{{ $admin->is_active ? 'default' : 'destructive' }}"
                                        class="{{ $admin->is_active ? 'bg-green-100 text-green-800 hover:bg-green-100' : '' }}">
                                ...
                                <x-shadcn.table-cell>
                                    <div class="flex flex-wrap items-center gap-2 text-xs sm:text-sm">
                                        @can('update', $admin)
                                            ...
                                            @if($admin->id !== auth()->id())
                                                <form method="POST" action="{{ route('admin.admins.toggle-active', $admin) }}"
                                                    class="inline">
                                                    @csrf
                                                    @method('PATCH')
                                                    <button type="submit"
                                                        class="{{ $admin->is_active ? 'text-orange-600 hover:text-orange-900' : 'text-green-600 hover:text-green-900' }} whitespace-nowrap">
                                        ...
                                        @can('impersonate', $admin)
                                            <form method="POST" action="{{ route('admin.admins.impersonate', $admin) }}"
                                                ...
                                                @csrf
                                                <button type="submit"
                                                    class="text-purple-600 hover:text-purple-900 whitespace-nowrap">Impersonate</button>
                                            </form>
                                        @endcan
                                        @can('delete', $admin)
                        ...
                        @empty
                            <x-shadcn.table-row>
                                <x-shadcn.table-cell colspan="5" class="text-center text-muted-foreground">No admins
