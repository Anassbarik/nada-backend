...
<div class="space-y-6">
  <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
    <div>
      ...
      <p class="text-gray-600 mt-1">Create a new flight and assign it to an event</p>
    </div>
    <a href="{{ route('admin.standalone.flights.index') }}" class="text-logo-link hover:underline inline-flex items-center">
  ...
  <x-shadcn.card class="shadow-lg">
    <x-shadcn.card-content class="p-6">
      <form method="POST" action="{{ route('admin.standalone.flights.store') }}" enctype="multipart/form-data">
        @csrf
        ...
        <div class="mb-6 p-4 border border-gray-300 rounded-md bg-blue-50">
          ...
          <div class="mb-4">
            ...
            <select id="beneficiary_type" name="beneficiary_type" class="block mt-1 w-full bg-white text-gray-900 border-gray-300 rounded-md shadow-sm" required onchange="toggleClientEmail()">
              <option value="organizer" {{ old('beneficiary_type', 'organizer') === 'organizer' ? 'selected' : '' }}>Event Organizer</option>
        ...
        @if(auth()->user()->isSuperAdmin() && isset($admins) && $admins->count() > 0)
          <div class="mb-6 p-4 border border-gray-300 rounded-md bg-gray-50">
            ...
            <p class="mt-1 mb-3 text-sm text-gray-600">
              Select admins who should be able to manage flights for the selected event.
            </p>
              ...
              @foreach($admins as $admin)
                <label class="flex items-center">
                  <input 
                    ...
                    value="{{ $admin->id }}"
                    {{ in_array($admin->id, old('flights_sub_permissions', [])) ? 'checked' : '' }}
