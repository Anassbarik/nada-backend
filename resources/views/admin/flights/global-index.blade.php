...
@section('content')
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                ...
                <p class="text-gray-600 mt-1 text-sm">
                    All flights across events. Each flight is associated with an event (accommodation) and shows its event
                    name.
                </p>
            </div>
    ...
    <x-shadcn.card class="mb-6">
        <x-shadcn.card-content class="p-6">
            <form action="{{ route('admin.standalone.flights.index') }}" method="GET"
                ...
                <div class="flex gap-2 w-full md:w-auto">
                    <button type="submit"
                        class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 w-full md:w-auto justify-center">
                        <i data-lucide="filter" class="w-4 h-4 mr-2"></i>
                        Filter
                    </button>
                    @if(request('accommodation_id'))
                        <a href="{{ route('admin.standalone.flights.index') }}"
                            class="inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md shadow-sm text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 w-full md:w-auto justify-center">
                            Clear
                        </a>
                    @endif
                </div>
                                            ...
                                            @if($flight->accommodation)
                                                <div class="flex justify-end gap-2">
                                                    ...
                                                    @if(auth()->user()->hasPermission('flights', 'create'))
                                                        <form method="POST"
                                                            action="{{ route('admin.standalone.flights.duplicate', $flight) }}"
                                                    ...
                                                    @if(auth()->user()->hasPermission('flights', 'delete'))
                                                        <form method="POST"
                                                            action="{{ route('admin.standalone.flights.destroy', $flight) }}"
