...
<div class="space-y-6">
  ...
  <x-shadcn.card class="shadow-lg">
    <x-shadcn.card-content class="p-6">
      <form method="POST" action="{{ route('admin.standalone.flights.update', $flight) }}" enctype="multipart/form-data">
        @csrf
        @method('PATCH')
          ...
          <div class="mb-4">
            ...
            <select id="accommodation_id" name="accommodation_id" class="block mt-1 w-full bg-white text-gray-900 border-gray-300 rounded-md shadow-sm" required>
              ...
              @foreach($accommodations as $acc)
                <option value="{{ $acc->id }}" {{ old('accommodation_id', $accommodation->id) == $acc->id ? 'selected' : '' }}>
                  {{ $acc->name }}
                </option>
              @endforeach
            </select>
